...
def add_user(login: str, nickname: str, password_hash, about: str):
    global conn, cur
...
def get_users_by_query(login: str, limit: int, excluded_id: int, offset=0):
    global conn, cur
    login = f"%{login}%"
    query = sql.SQL("""SELECT id, login, nickname  FROM users 
                       ...
                       LIMIT %s OFFSET %s;
                    """)
    cur.execute(query, (login, login, excluded_id, limit, offset))
...
def change_attribute_by_id(user_id: int, attribute: str, new_attribute: str):
    global conn, cur

    query = sql.SQL("""UPDATE users SET {attribute} = %s WHERE id = %s;""").format(
        attribute=sql.Identifier(attribute),
    )
    cur.execute(query, (new_attribute, user_id))
...
def chat_is_exists(chat_id: int) -> bool:
    global conn, cur
...
def create_chat(user_main_id: int, user_chatter_id: int) -> int:
    global conn, cur
...
def track_message(user_main_id: int, chat_id: int, message: str) -> int:
    global conn, cur
...
def track_message_and_create_chat(sender_id: int , reciever_id: int, msg_body: str) -> int:
    chat_id = (create_chat(sender_id, reciever_id))
    track_message(sender_id, chat_id, msg_body)
    return chat_id


def get_user_chats(user_id: int, limit : int, offset=0):
    global conn, cur
...
def get_messages_from_user_from_chat(user_id: int, chat_id: int, limit: int, offset=0):
    global conn, cur
...
def user_is_banned(user_id: int):
    global conn, cur
...
def get_messages(chat_id: int, limit: int, offset=0):
    global conn, cur
...
def get_chat_by_users(user_main_id: int, user_chatter_id: int):
    global conn, cur
...
def set_message_is_read(chat_id: int):
    global conn, cur

    cur.execute("""UPDATE messages SET is_read = True WHERE id = %s;""", (chat_id, ))
    conn.commit()


def get_first_message(chat_id: int):
    global conn, cur
